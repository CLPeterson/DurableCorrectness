...
void WorkThread(uint32_t numThread, int threadId, uint32_t testSize, uint32_t tranSize, uint32_t keyRange, uint32_t insertion, uint32_t deletion, ThreadBarrier& barrier,  T& set)
{
    ...
    ssmem_alloc_init(alloc, SSMEM_DEFAULT_MEM_SIZE, threadId);   
...
int main(int argc, const char *argv[])
{
    ...
    case 0:
        { 
			SetAdaptor<LinkFreeList<intptr_t>> set(testSize, numThread, tranSize, keyRange); Tester(numThread, testSize, tranSize, keyRange, insertion, deletion, set); 
		}
        break;
	case 1:
         { SetAdaptor<LinkFreeSkipList<intptr_t>> set(testSize, numThread, tranSize, keyRange); Tester(numThread, testSize, tranSize, keyRange, insertion, deletion, set);  }
         break;
	case 2:
         { SetAdaptor<LinkFreeHashTable<intptr_t>> set(testSize, numThread, tranSize, keyRange); Tester(numThread, testSize, tranSize, keyRange, insertion, deletion, set);  }
         break;
    // case 1:
