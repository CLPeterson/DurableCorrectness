...
public:
    ...
    bool insert(intptr_t k, T item, int tid)
    {
		...
		long int invocation; //CORRECTNESS ANNOTATIONS	
    ...
    retry:
        ...
        if (found)
        {
			...
			create_method_persist(k, INT_MIN, SET, PRODUCER, invocation, response, false); //CORRECTNESS ANNOTATIONS
        ...
        for (int i = 1; i < newNode->topLevel; i++)
        {
            while (true)
            {
				...
				{
					//printf("Insert %ld\n", k);
					create_method_persist(k, INT_MIN, SET, PRODUCER, invocation, response, true); //CORRECTNESS ANNOTATIONS
					insert_method_persist();
                    return true;
				}
    ...
    bool remove(intptr_t k, int tid)
    {
        ...
        if (!found)
		{
			...
			invocation = get_elapsed_nanoseconds(); //CORRECTNESS ANNOTATIONS	
			response = get_elapsed_nanoseconds(); //CORRECTNESS ANNOTATIONS
			create_method_persist(k, INT_MIN, SET, CONSUMER, invocation, response, false); //CORRECTNESS ANNOTATIONS
			insert_method_persist();
            return false;
		}
        ...
        if (result)
        {
			...
			//printf("Remove %ld\n", k);
			create_method_persist(k, INT_MIN, SET, CONSUMER, invocation, response, true); //CORRECTNESS ANNOTATIONS
			insert_method_persist();
            return true;
        }
		//printf("Remove %ld fail\n", k);
        ...
        for (int i = MAX_LEVEL - 1; i >= 0; i--)
        {
            ...
            while (curr->key < k || linkFreeUtils::isMarked(curr->next[i].load()))
            {
                ...
                else if (i == 0 && curr->key == k)
                {
                    ...
                    FLUSH_DELETE(curr);
					response = get_elapsed_nanoseconds(); //CORRECTNESS ANNOTATIONS
					create_method_persist(k, INT_MIN, SET, READER, invocation, response, false); //CORRECTNESS ANNOTATIONS
					insert_method_persist();
                    return false;
                }
            ...
            if (curr->key == k)
            {
                ...
                FLUSH_INSERT(curr);
				response = get_elapsed_nanoseconds(); //CORRECTNESS ANNOTATIONS
				create_method_persist(k, INT_MIN, SET, READER, invocation, response, true); //CORRECTNESS ANNOTATIONS
				insert_method_persist();
                return true;
            }
        }
