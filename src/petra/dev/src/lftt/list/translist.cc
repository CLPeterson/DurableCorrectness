...
(
	...
	//Method** m1 = new Method*[TRANS_SIZE]; //CORRECTNESS ANNOTATIONS
	//Method** m1_persist = new Method*[TRANS_SIZE]; //CORRECTNESS ANNOTATIONS
    ...
    while(desc->status == ACTIVE && ret != FAIL && opid < desc->size)
    {
        ...
        if(op.type == INSERT)
        {
			...
			if(ret == FAIL) method_ret = false; //CORRECTNESS ANNOTATIONS
			create_method(op.key, INT_MIN, SET, PRODUCER, invocation, response, method_ret); //CORRECTNESS ANNOTATIONS
			//create_method_persist(op.key, INT_MIN, SET, PRODUCER, invocation, response, method_ret); //CORRECTNESS ANNOTATIONS
        }
        else if(op.type == DELETE)
        {
			...
			//create_method_persist(op.key, INT_MIN, SET, CONSUMER, invocation, response, method_ret); //CORRECTNESS ANNOTATIONS
        }
        else
        {
			...
			//create_method_persist(op.key, INT_MIN, SET, READER, invocation, response, method_ret); //CORRECTNESS ANNOTATIONS		
        }
...
inline void TransList::HelpOps_rec(Desc* desc, uint8_t opid)
{
    if(desc->status != ACTIVE)
    {
        bool needHelp = false;
...
inline void TransList::FinishPendingTxn(NodeDesc* nodeDesc, Desc* desc)
{
	...
	HelpOps_rec(nodeDesc->desc, nodeDesc->opid + 1); //CORRECTNESS ANNOTATIONS
}
...
void TransList::validate_and_recovery(int thread_count, Allocator<TransList::Desc>* l_descAllocator)
{
    ...
    while(curr != m_tail) {
		...
		create_method_persist(curr->key, INT_MIN, SET, PRODUCER, invocation, response, true); //CORRECTNESS ANNOTATIONS			
		insert_txn_persist(invocation, response, 1); //CORRECTNESS ANNOTATIONS
