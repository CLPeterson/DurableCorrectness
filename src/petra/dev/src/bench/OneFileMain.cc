...
if(VERBOSE) {
	...
	vsv_exit(); //CORRECTNESS ANNOTATIONS
}
...
int main(int argc, const char *argv[])
{
	...
	vsv_args(argc, argv); //CORRECTNESS ANNOTATIONS
	vsv_init(); //CORRECTNESS ANNOTATIONS

    const char* setName[] = 
    {        "OneFileTMLinkedListSet",
        "OneFileTMSkipList",
        "OneFileOFMDListSet",
        "OneFileTMHashMap",
        // "OneFileTMLinkedListSet",
    ...
    case 0:
        { 
		SetAdaptor<TMLinkedListSet<int,poflf::OneFileLF,poflf::tmtype>> set(testSize, numThread + 1, tranSize, keyRange); Tester(numThread, testSize, tranSize, keyRange, insertion, deletion, set);
		}
        break;
    case 1:
        { SetAdaptor<TMSkipList<int,poflf::OneFileLF,poflf::tmtype>> set(testSize, numThread + 1, tranSize, keyRange); Tester(numThread, testSize, tranSize, keyRange, insertion, deletion, set); }
        break;                  
    case 2:
        { SetAdaptor<OFMDListSet<int,poflf::OneFileLF,poflf::tmtype>> set(testSize, numThread + 1, tranSize, keyRange); Tester(numThread, testSize, tranSize, keyRange, insertion, deletion, set); }
        break;
    case 3:
        { MapAdaptor<TMHashMap<int,int, poflf::OneFileLF,poflf::tmtype>> map(testSize, numThread + 1, tranSize, keyRange); MapTester(numThread, testSize, tranSize, keyRange, insertion, deletion, update, map); }
        break;                          
    // case 4:
	...
	//print_thread_lists(); //CORRECTNESS ANNOTATIONS
	//print_thread_lists_persist(); //CORRECTNESS ANNOTATIONS
