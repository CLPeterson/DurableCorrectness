...
class RomulusLR {
    ...
    static const int CHUNK_SIZE = 2048*1024;

    // Filename for the mapping file
#if defined USE_PMEM_ALLOCATOR
    const char* MMAP_FILENAME = "/mnt/pmem0/romuluslr_shared";
#else
...
private:
    ...
    inline void flush_range(uint8_t* addr, size_t length) {
		...
		for (; ptr < last; ptr += cache_line_size) {
			...
			//printf("flush_range PWB(%p)\n", ptr); //CORRECTNESS ANNOTATIONS
		}
		//printf("End flush_range\n");
    }
    void copyMainToBack() {
...
public:
    ...
    void ns_init(){
		...
		max_size = 12*1024*1024*1024ULL;//400*1024*1024; // 400 Mb => 200 Mb for the user
    ...
    void createFile(){
        ...
        // mmap() memory range
        void* got_addr = pmem_map_file(MMAP_FILENAME, max_size, flags, mode, &mapped_len, &is_pmem);        
        if (got_addr == MAP_FAILED) {
            printf("got_addr = %p  %p\n", got_addr, MAP_FAILED);
            perror("ERROR: mmap() is not working !!! ");
            assert(false);
        ...
        for (int i = 0; i < 100; i++) {
			...
			PWB(&per->objects[i]);
			//printf("PWB(%p)\n", &per->objects[i]); //CORRECTNESS ANNOTATIONS
        }
        end_transaction();
        ...
        per->id = MAGIC_ID;	
		PWB(&per->id); 
		//printf("PWB(%p)\n", &per->id); //CORRECTNESS ANNOTATIONS
        PSYNC();
#else
        ...
        for (int i = 0; i < 100; i++) {
			...
			PWB(&per->objects[i]); 
			//long int response = get_elapsed_nanoseconds(); //CORRECTNESS ANNOTATIONS
			//handle_PWB(&per->objects[i], invocation, response); //CORRECTNESS ANNOTATIONS
			//printf("PWB(%p)\n", &per->objects[i]); //CORRECTNESS ANNOTATIONS
        }
        end_transaction();
		...
		PWB(&per->id); 
		//long int response = get_elapsed_nanoseconds(); //CORRECTNESS ANNOTATIONS
		//handle_PWB(&per->id, invocation, response); //CORRECTNESS ANNOTATIONS
		//printf("PWB(%p)\n", &per->id); //CORRECTNESS ANNOTATIONS
        PSYNC();
#endif
    }
    ...
    static inline void put_object(int idx, T* obj) {
		...
		//long int response = get_elapsed_nanoseconds(); //CORRECTNESS ANNOTATIONS
		//handle_PWB(&(gRomLR.per->objects[idx]), invocation, response); //CORRECTNESS ANNOTATIONS
		//printf("PWB(%p)\n", &(gRomLR.per->objects[idx])); //CORRECTNESS ANNOTATIONS
    }
    ...
    inline void begin_transaction() {
		...
		PWB(&per->state);
		//printf("PWB(%p)\n", &per->state); //CORRECTNESS ANNOTATIONS
    ...
    inline void end_transaction() {
		...
		//printf("PWB(%p)\n", &per->state); //CORRECTNESS ANNOTATIONS
		PWB(&per->used_size);
		//printf("PWB(%p)\n", &per->used_size); //CORRECTNESS ANNOTATIONS
    ...
    void ns_write_transaction(Func&& mutativeFunc) {
		...
		PWB(&per->state);
		//long int response = get_elapsed_nanoseconds(); //CORRECTNESS ANNOTATIONS
		//handle_PWB(&per->state, invocation, response); //CORRECTNESS ANNOTATIONS
		//printf("PWB(%p)\n", &per->state); //CORRECTNESS ANNOTATIONS
        // One PFENCE() is enough for all user modifications because no ordering is needed between them.
        PFENCE();
        g_right = true;
		...
		//invocation = get_elapsed_nanoseconds(); //CORRECTNESS ANNOTATIONS	
		PWB(&per->state);
    ...
    static T* alloc(Args&&... args) {
        ...
        if (r.per->used_size < (uint8_t*)addr - r.main_addr + sizeof(T) + 128) {
			...
			PWB(&r.per->used_size);
			//long int response = get_elapsed_nanoseconds(); //CORRECTNESS ANNOTATIONS
			//handle_PWB(&r.per->used_size, invocation, response); //CORRECTNESS ANNOTATIONS
			//printf("PWB(%p)\n", &r.per->used_size); //CORRECTNESS ANNOTATIONS
        }
        return ptr;
    }
