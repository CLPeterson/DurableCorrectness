...
public:
  ...
  bool add(K key) {
    return TM::template updateTx<bool>([=]() {
		...
		//printf("New node %p\n", new_node); //CORRECTNESS ANNOTATIONS
		insert_persist_map(new_node, (int) key, INT_MIN, SET, PRODUCER); //CORRECTNESS ANNOTATIONS
  ...
  bool remove(K key) {
    return TM::template updateTx<bool>([=]() {
		...
		insert_persist_map(curr, (int) key, INT_MIN, SET, CONSUMER); //CORRECTNESS ANNOTATIONS
        TM::tmDelete(curr);
        return true;
    });
  }
