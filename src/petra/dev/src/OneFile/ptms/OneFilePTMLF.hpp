...
//static const uint64_t TX_MAX_STORES = 64*1024; //Non-Persistent Memory
...
#if defined USE_PMEM_ALLOCATOR
static const char * PFILE_NAME = "/mnt/pmem0/ponefilelf_shared";
#else
...
//static const uint64_t PREGION_SIZE = 2*1024*1024*1024ULL;   //Non-Persistent Memory
...
static inline void flushFromTo(void* from, void* to) noexcept {
	...
	for (; ptr < (uint8_t*)to; ptr += cache_line_size) { 
		...
		handle_PWB(ptr, invocation, response); //CORRECTNESS ANNOTATIONS
		//printf("PWB(%p)\n", ptr); //CORRECTNESS ANNOTATIONS
    ...
    void applyFromRecover() {
        ...
        for (uint64_t i = 0; i < numStores; i++) {
            *((uint64_t*)plog[i].addr) = plog[i].val;
			...
			PWB(plog[i].addr); 
			long int response = get_elapsed_nanoseconds(); //CORRECTNESS ANNOTATIONS
			handle_PWB(plog[i].addr, invocation, response);
			//printf("PWB(%p)\n", plog[i].addr); //CORRECTNESS ANNOTATIONS
        }
    }
};
...
struct WriteSet {
    ...
    inline void persistAndFlushLog(PWriteSet* const pwset) {
        for (uint64_t i = 0; i < numStores; i++) {
			...
			//printf("log[%d].addr = %p, log[%d].val = %u\n", i, log[i].addr, i, log[i].val); //CORRECTNESS
        }
    ...
    inline void flushModifications() {
		...
		for (uint64_t i = 0; i < numStores; i++) { 
			...
			PWB(log[i].addr);  
			long int response = get_elapsed_nanoseconds(); //CORRECTNESS ANNOTATIONS
			handle_PWB(log[i].addr, invocation, response);
			//printf("PWB(%p)\n", log[i].addr); 
		} //CORRECTNESS
    }
...
public:
    ...
    void mapPersistentRegion(const char* filename, uint8_t* regionAddr, const uint64_t regionSize) {
        ...
        if (stat(filename, &buf) == 0) {
            ...
            void* got_addr = pmem_map_file(filename, regionSize, flags, mode, &mapped_len, &is_pmem);
            if (got_addr == MAP_FAILED) {
                printf("got_addr = %p  instead of %p\n", got_addr, regionAddr);
                perror("ERROR: mmap() is not working !!! ");
                assert(false);
            }
            regionAddr = (uint8_t *)got_addr;
            PREGION_ADDR = regionAddr;
            PREGION_END = (PREGION_ADDR+PREGION_SIZE);
            ...
            if (reuseRegion) {
				...
				PWB(&pmd->id);  
				//printf("PWB(%p)\n", &pmd->id); //CORRECTNESS ANNOTATIONS
                PFENCE();
            }
        }
        // mmap() memory range
    ...
    inline bool commitTx(OpData& myopd, const int tid) {
		...
		//printf("Thread %d: commitTx seq %u\n", tid, trans2seq(myopd.curTx));
		long int invocation = get_elapsed_nanoseconds(); //CORRECTNESS ANNOTATIONS	
		PWB(curTx); 
		long int response = get_elapsed_nanoseconds(); //CORRECTNESS ANNOTATIONS
		handle_PWB(curTx, invocation, response);
		//printf("PWB(%p)\n", curTx); //CORRECTNESS
    ...
    template<typename R, typename F> R transaction(F&& func) {
        ...
        tl_opdata = &myopd;
        R retval {};
		//int cnt = 0; //CORRECTNESS
		//long int txn_invocation; //CORRECTNESS ANNOTATIONS 
		//long int txn_response; //CORRECTNESS ANNOTATIONS 
		long int txn_invocation_persist; //CORRECTNESS ANNOTATIONS 
		long int txn_response_persist; //CORRECTNESS ANNOTATIONS 
        while (true) {
			//printf("Thread %d: BeginTxn Iteration %d\n", tid, ++cnt); //CORRECTNESS
			//printf("Thread %d: beginTx seq %u\n", tid, trans2seq(myopd.curTx));
            beginTx(myopd, tid);
            try {
				//txn_invocation = get_elapsed_nanoseconds(); //CORRECTNESS ANNOTATIONS
                retval = func();
				//txn_response = get_elapsed_nanoseconds(); //CORRECTNESS ANNOTATIONS
				if (!retval)
				{ 
					rollback_txn(); //CORRECTNESS ANNOTATIONS
					rollback_txn_persist(); //CORRECTNESS ANNOTATIONS
					break; //CORRECTNESS ANNOTATIONS
				}
            } catch (AbortedTx&) {
				rollback_txn(); //CORRECTNESS ANNOTATIONS
				rollback_txn_persist(); //CORRECTNESS ANNOTATIONS
			...
			txn_invocation_persist = get_elapsed_nanoseconds(); //CORRECTNESS ANNOTATIONS
            if (commitTx(myopd, tid)) break;
			else { //CORRECTNESS ANNOTATIONS
				rollback_txn(); //CORRECTNESS ANNOTATIONS
				rollback_txn_persist(); //CORRECTNESS ANNOTATIONS
			}
        }
		if(retval) //CORRECTNESS ANNOTATIONS
		{
			txn_response_persist = get_elapsed_nanoseconds(); //CORRECTNESS ANNOTATIONS
			//insert_txn(txn_invocation, txn_response, TRANS_SIZE); //CORRECTNESS ANNOTATIONS
			insert_txn_persist(txn_invocation_persist, txn_response_persist, TRANS_SIZE); //CORRECTNESS ANNOTATIONS
		}
        tl_opdata = nullptr;
        --myopd.nestedTrans;
        return retval;
