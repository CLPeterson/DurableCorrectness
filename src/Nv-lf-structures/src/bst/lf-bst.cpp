...
//#define BUFFERING_ON //CORRECTNESS ANNOTATIONS
...
svalue_t bst_search(skey_t key, NODE_PTR node_r, EpochThread epoch, linkcache_t* buffer) {
   ...
   if (ADDRESS(seek_record->leaf)->key == key) {
	...
	flush_and_try_unflag((PVOID*)&(seek_record->parent->left)); //CORRECTNESS ANNOTATIONS
#endif
		...
		insert_method_persist(); //CORRECTNESS ANNOTATIONS
