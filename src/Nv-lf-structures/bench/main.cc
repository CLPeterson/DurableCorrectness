...
void WorkThread(uint32_t numThread, int threadId, uint32_t testSize, uint32_t tranSize, uint32_t keyRange, uint32_t insertion, uint32_t deletion, ThreadBarrier& barrier,  T& set)
{
	...
	EpochThread epoch = EpochThreadInit(threadId);
...
template<typename T>
void TesterRecover(uint32_t numThread, uint32_t testSize, uint32_t tranSize, uint32_t keyRange, uint32_t insertion, uint32_t deletion,  SetAdaptor<T>& set, bool recovery = false)
{
    std::vector<std::thread> thread(numThread);
    ThreadBarrier barrier(numThread + 1);
    if(!recovery) {
		...
		set.validate_and_recovery(numThread); //CORRECTNESS ANNOTATIONS
...
#if LINKEDLIST_T
	SetAdaptor<linkedlist_t*> set(testSize, numThread, tranSize, keyRange); Tester(numThread, testSize, tranSize, keyRange, insertion, deletion, set); 
#elif SKIPLIST_T
	SetAdaptor<skiplist_t*> set(testSize, numThread, tranSize, keyRange); 
Tester(numThread, testSize, tranSize, keyRange, insertion, deletion, set); 
#elif BST_T
	SetAdaptor<NODE_PTR> set(testSize, numThread, tranSize, keyRange); Tester(numThread, testSize, tranSize, keyRange, insertion, deletion, set); 
#elif HASHTABLE_T
	SetAdaptor<ht_intset_t*> set(testSize, numThread, tranSize, keyRange); Tester(numThread, testSize, tranSize, keyRange, insertion, deletion, set); 
#endif
